"""Synthetic SPHEREx cutout + catalog fixtures (no network, no real data).

``make_synth_cutout`` writes a spec-compliant MEF with injected point/galaxy
sources of known mJy flux; ``make_synth_catalog`` writes the matching reference
catalog. Used across the test suite to exercise parsing, backends, and the
pipeline end-to-end without downloading anything.
"""
...
PIXSCALE = 6.15  # arcsec/native pixel
OMEGA_SR = ((PIXSCALE * u.arcsec) ** 2).to_value(u.sr)
IMG_SCALE = 1.0e9  # MJy -> mJy (must match spherex_photometry.constants.IMG_SCALE)
